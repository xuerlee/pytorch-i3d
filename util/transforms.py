...
class Normalize(object):
    ...
    def __call__(self, tensor,bbox):
        ...
        """
        return F.normalize(tensor, self.mean, self.std, self.inplace),bbox
...
class CenterCrop(object):
    ...
    def __call__(self, img,bbox):
        ...
        bbox[:, 0] =  torch.clamp(bbox[:, 0] - j,0,tw-1)
        bbox[:, 1] =  torch.clamp(bbox[:, 1] - i,0,th-1)
        bbox[:, 2] =  torch.clamp(bbox[:, 2] - j,0,tw-1)
        bbox[:, 3] =  torch.clamp(bbox[:, 3] - i,0,th-1)



        return F.center_crop(img, self.size),bbox
...
class RandomCrop(object):
    ...
    def __call__(self, img,bbox):
        ...
        bbox[:, 0] =  torch.clamp(bbox[:, 0] - j,0,w-1)
        bbox[:, 1] =  torch.clamp(bbox[:, 1] - i,0,h-1)
        bbox[:, 2] =  torch.clamp(bbox[:, 2] - j,0,w-1)
        bbox[:, 3] =  torch.clamp(bbox[:, 3] - i,0,h-1)

        return F.crop(img, i, j, h, w),bbox
